...
describe('clamp (expected correct behaviour)', () => {
    ...
    test.skip('clamps values to the upper bound', () => {
        const result = clamp(10, -5, 5);
        expect(result).toBe(5);
    });

    test.skip('returns the number if within bounds', () => {
        const result = clamp(0, -5, 5);
        expect(result).toBe(0);
    });

    test.skip('resolves parameters as 0 if lower bound is NaN', () => {
        const result = clamp(10, NaN, 5);
        expect(result).toBe(5);
    });
    ...
    test.skip('handles floating point numbers', () => {
        const result = clamp(10.5, -5.5, 5.5);
        expect(result).toBe(5.5);
    });
});